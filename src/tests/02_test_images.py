...
def test_api_get_latest_images_check_number_result_low_or_equals_3(test_app):
    ...
    assert latest_images is not None, \
        'Bad type of response, a list was expected, but None came'

    assert len(latest_images) <= 3, \
        f'Expected len(latest_image) <= 3, but {len(latest_images)} came'
...
def test_api_post_negative_image_check_incorrect_extensions(test_app):
    ...
    for filename in filenames:
        ...
        assert response.status_code == 422, \
            f"Bad extension, status code {response.status_code} != 422"
...
def test_api_post_negative_image_check_correct_extensions(test_app):
    ...
    for filename in filenames:
        ...
        assert response.status_code == 201, \
            f"Bad extension, status code {response.status_code} != 201"
    shutil.rmtree('tmp')
...
def test_api_negative_image_check_result_image_color(test_app):
    ...
    for pixels in pixel_cases:
        ...
        assert (255 - pixels[0][0][0]) == new_pixels[0], \
            'Bad inverted color Red'
        assert (255 - pixels[0][0][1]) == new_pixels[1], \
            'Bad inverted color Green'
        assert (255 - pixels[0][0][2]) == new_pixels[2], \
            'Bad inverted color Blue'
